			...
			uploadify:function(options,swfUploadOptions) {
				jQuery(this).each(function() {
					...
					function onDialogOpen() {
						...
						swfuploadify.queue.errorMsg       = 'Файлы не загружены';
						...
						if (jQuery.inArray('onSelect',swfuploadify.settings.skipDefault) < 0) {
							...
							for (var n in swfuploadify.queue.files) {
								...
								if (queuedFile.name == file.name) {
									var replaceQueueItem = confirm('Файл "' + file.name + '" уже есть в очереди.\nВы хотите его заменить?');
									if (!replaceQueueItem) {
							...
							switch(errorCode) {
								case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
									if (swfuploadify.settings.queueSizeLimit > errorMsg) {
										swfuploadify.queue.errorMsg += '\nИзвините, ограничение на количество изображений (Осталось: ' + errorMsg + ').';
									} else {
										swfuploadify.queue.errorMsg += '\nThe number of files selected exceeds the queue size limit (' + swfuploadify.settings.queueSizeLimit + ').';
									}
									break;
								case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
									swfuploadify.queue.errorMsg += '\nФайл "' + file.name + '" превышает размер (' + swfuploadify.settings.fileSizeLimit/1000 + 'Мб).';
									break;
								case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
									swfuploadify.queue.errorMsg += '\nФайл "' + file.name + '" пустой.';
									break;
								case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
									swfuploadify.queue.errorMsg += '\nФайл "' + file.name + '" имеет запрещенный тип (' + swfuploadify.settings.fileTypeDesc + ').';
									break;
							}
						}
									...
									case SWFUpload.FILE_STATUS.COMPLETE:
										setTimeout(function() { 
											if (jQuery('#' + file.id)) {
												swfuploadify.queue.queueSize -= file.size;
												delete swfuploadify.queue.files[file.id]
												jQuery('#' + file.id).fadeOut(1000,function() {
													jQuery(this).remove();
												});
											}
						...
						switch(errorCode) {
							case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
								if (errorMsg != '500')
                                                                    ...
                                                                    errorString = 'La imagen no cumple con los requerimientos';                                                                
								break;
					...
					function onUploadSuccess(file,data,response) {
                                                ...
                                                if (data == '') {
                                                    ...
                                                    $('#' + file.id).find('.data').html(' - La imagen no cumple con los requisitos');
                                                    return false;
                                                }
						swfuploadify.queue.queueBytesUploaded += file.size;
						jQuery('#' + file.id).find('.data').html(' - Cargado');
