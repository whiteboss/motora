...
(function($) {
	...
	var _createHtml = function(pointer) {
	    ...
	    html += (settings.allowAdd) ? '<a href="javascript:void(0);" class="add-note" /></a>' : '';
	    
	    html += (settings.allowHide && settings.loadNotes) ? '<a href="javascript:void(0);" class="hide-notes" />' : '';
	    
	    html += (settings.allowReload && settings.loadNotes) ? '<a href="javascript:void(0);" class="reload-notes" />' : '';
	...
	var _getNotes = function(pointer) {
		...
		$.ajax({
		    ...
		    type: 'POST',
		    beforeSend: function() {
			(ID.firstLoad) ? _startLoading(pointer, 'cargando etiquetas') : '';
		    },
	...
	var _printNote = function(pointer, note) {
	    ...
	    //html += (note.NOTE != '') ? note.NOTE : note.LINK;
            //html += (note.title != '') ? note.title : '';
	    
	    //html += '</span>';
	    
	    html += (note.facebook_name != '') ? '<span class="author"><a href="https://facebook.com/profile.php?id='+note.uid+'" target="_blank">'+note.facebook_name+'</a></span>' : '';
	    ...
	    var position = {
		'left': _percentToPixel(pointer, note.x, 'left'),
		'top': _percentToPixel(pointer, note.y, 'top'),
		'width': _percentToPixel(pointer, note.w, 'width'),
		'height': _percentToPixel(pointer, note.h, 'height')
	    };
...
//		}
	...
	var _setNote = function(pointer, event) {
		...
		// Qlick
		html += (settings.allowAuthor) ? '<br /><span id="friend_id"><img src="/sprites/30.gif" width="48" height="6" alt="" /> &nbsp; Buscamos sus amigos...</span><br />' : '';
		
		//html += '<input type="text" name="title" value="Escribe algo..."><br />';
		
		//html += (settings.allowLink) ? '<input type="text" name="link" value="http://" /><br />' : '';
		
		html += '<a href="javascript:void(0);" class="save-note" title="Guardar la etiqueta"></a><a href="javascript:void(0);" class="cancel-note" title="Cancelar"></a></div>';
		...
		$('#jquery-notes_'+pointer+' .notes .select').append(html);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data) {
                            $('#friend_id').replaceWith('<select id="friend_id" name="friend_id" data-placeholder="Selecciona un nombre..." class="chzn-select" /></select>');                            
                            $.each(data, function(i, value) {
                                name = value.name;
                                id = value.id;
                                $('#friend_id').append($('<option>').text(name).attr('value', id));
                            });
                            $("#friend_id").chosen({width: "300px"});
                        }
                    }
                });                
                // ------------                
...
//            var title = $('#jquery-notes_'+pointer+' .notes .select .text-box input[name="title"]').val();
            var friend_id = $('#jquery-notes_'+pointer+' .notes .select .text-box select[name="friend_id"] :selected').val();
            $('#friend_id').attr('disabled', 'disabled');
...
//	    title = (title == undefined) ? '' : title;
		...
		//data: operation+'=true&image='+ID.image+'&id='+id+'&position='+position.left+','+position.top+','+position.width+','+position.height+'&note='+note+'&link='+link+'&author='+author,
                data: operation+'=true&image='+ID.image+'&position='+(position.left + 7)+','+(position.top + 9)+','+position.width+','+position.height+'&friend_id='+friend_id,
		success: function(data) {
	...
	var _hideNotes = function(pointer) {
		...
		if ($('#jquery-notes_'+pointer+' .controller .hide-notes').hasClass('show-notes')) {
			...
			visibility: 'visible'
                    ...
                    // qlick
                    $('#jquery-notes_'+pointer+' .notes .text').css({
			visibility: 'visible'
		    });                    
