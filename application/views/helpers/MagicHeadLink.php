 ...
 */
class Application_Views_Helper_MagicHeadLink extends Zend_View_Helper_HeadLink
{
...
//                $headLink->appendStylesheet('/css/bootstrap_test.css', 'screen, projection');
//            else
//                $headLink->appendStylesheet('/css/bootstrap_mobile.css', 'screen, projection');
//
//        } else {
            $headLink->appendStylesheet('/css/bootstrap1808.css', 'screen, projection');
//        }
    ...
    private function getCompiledItem()
    {
        ...
        $path = $_SERVER['DOCUMENT_ROOT'] . '/cache/' . $filename . (self::$compress? '_compressed' : '') . '.css';
        if (!file_exists($path)) {
            ...
            foreach ($this->_cache as $css) {
                ...
                if (self::$compress) {
                    $cssContent .= Qlick_Minify_CSS::minify(
                        $content, 
                        array(
                    ...
                    $cssContent .= Qlick_Minify_CSS_UriRewriter::rewrite(
                        $content, 
