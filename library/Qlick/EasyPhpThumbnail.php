	...
	private function addpngwatermark() {
		...
		if (file_exists($this->Watermarkpng)) {
			...
			$wpos=explode(' ',str_replace('%','',$this->Watermarkposition));
                        imagecopy($this->im,$this->newimage,min(max(imagesx($this->im)*($wpos[0]/100)-0.5*imagesx($this->newimage),0),imagesx($this->im)-imagesx($this->newimage)),min(max(imagesy($this->im)*($wpos[1]/100)-0.5*imagesy($this->newimage),0),imagesy($this->im)-imagesy($this->newimage)),0,0,imagesx($this->newimage),imagesy($this->newimage));
                        // whiteboss. qlick
                        // imagecopymerge - не поддерживает прозрачный фон
...
//                if ($radius == 0) {  
//                    return $this->newimage; imagedestroy($this->newimage); break;
//                }  
//                $w = imagesx($this->newimage); $h = imagesy($this->newimage);  
...
//                        array( 1, 2, 1 )   
//                    );   
//                    imagecopy ($imgBlur, $this->newimage, 0, 0, 0, 0, $w, $h);  
...
//                    for ($i = 0; $i < $radius; $i++)    {  
//                        imagecopy ($imgBlur, $this->newimage, 0, 0, 1, 0, $w - 1, $h); // left  
//                        imagecopymerge ($imgBlur, $this->newimage, 1, 0, 0, 0, $w, $h, 50); // right  
//                        imagecopymerge ($imgBlur, $this->newimage, 0, 0, 0, 0, $w, $h, 50); // center  
...
//
//                            $rgbOrig = ImageColorAt($this->newimage, $x, $y);  
...
//                                    $pixCol = ImageColorAllocate($this->newimage, $rNew, $gNew, $bNew);  
//                                    ImageSetPixel($this->newimage, $x, $y, $pixCol);  
//                                }  
//                        }  
//                    }  
//                }  
...
//                                ImageSetPixel($this->newimage, $x, $y, $rgbNew);  
//                        }  
//                    }  
//                }  
...
//        */
//
//        function UnsharpMask($img, 100, 0.3, 0)    {
//
//        ////////////////////////////////////////////////////////////////////////////////////////////////
//        ////
//        ////                  Unsharp Mask for PHP - version 2.1.1
