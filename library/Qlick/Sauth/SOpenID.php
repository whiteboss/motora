 ...
 */
class SOpenID extends CBaseUserIdentity
{
    public $returnUrl
         , $identity
         , $required = array()
    ...
    private $claimed_id, $isAuthenticated=false;
    ...
    protected function request($url, $method='GET', $params=array()) {
      ...
      if(isset($http_response_header))$head = implode("\r\n", $http_response_header); 
      else{
        $head = $this->getCurlRequest($url, $method, $params);  
        $body = '';
      } 
    ...
    protected function getCurlRequest($url, $method='GET', $options=array()) {
      $ch = curl_init();    
      ...
      if (curl_errno($ch) > 0){
        throw new SocialException(curl_error($ch), curl_errno($ch));
      }      
