 ...
 */
abstract class Qlick_Sauth_SAuthServiceBase implements Qlick_Sauth_IAuthService {
...
//  }
  ...
  public function redirect_w($url = null) {
    $this->component->redirect(isset($url) ? $url : $this->redirectUrl, true);
  }
   ...
   * @return cURL handle.
   */
  protected function initRequest($data) {
    $ch = curl_init();    
   ...
   */
  protected function makeRequest($url, $options = array(),  $method = 'POST') {
    $ch =  $this->initRequest($options);
    ...
    if (curl_errno($ch) > 0){
      throw new Qlick_Sauth_SocialException(curl_error($ch), curl_errno($ch));
    }      
    ...
    if ($headers['http_code'] != 200){
      ...
      var_dump($result);
      exit;
//      throw new SocialException('Invalid response http code: '.$headers['http_code'].'. '. $headers['http_code']);      
    }
    ...
    //die();
    //if ($result <> '') $result = $this->parseJson($result);
  ...
  protected function parseJson($response) {
    try {
      ...
      if (!isset($result)) {
        throw new Qlick_Sauth_SocialException('Invalid response format.');
      }
      else if (isset($error)) {
        throw new Qlick_Sauth_SocialException($error['message'] .' == '. $error['code']);
      }
      else
        return $result;
    }
   ...
   */
  protected function resetState($key) {
    $key = $this->getStateKeyPrefix().$key;
    unset($_SESSION[$key]);
  }
  ...
  public function getAuthToken() {
    if($this->hasState('auth_token')){
      return $this->getState('auth_token');
    }
    else{
      return false;
    }    
