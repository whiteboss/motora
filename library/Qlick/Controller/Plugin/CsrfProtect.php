 ...
 */
class Qlick_Controller_Plugin_CsrfProtect extends Zend_Controller_Plugin_Abstract 
{
	/**
	 * Session storage
	...
	{	
		...
		{			
			if(empty($this->_previousToken))
				throw new Exception('A possible CSRF attack detected - no token received');

			$value = $request->getPost($this->_keyName);
			if(!$this->isValidToken($value))
				throw new Exception('A possible CSRF attack detected - tokens do not match');
		}			
	}
